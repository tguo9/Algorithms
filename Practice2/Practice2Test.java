...
public class Practice2Test {
	...
	public Practice2Test(String[] args) {
		...
		try {
			...
			// This is not strictly true. For example, the setting the array size may work
			// and
			usage();
		}
	...
	public void timeSearches() {
		...
		System.out.println(search.searchName() + ": total search time = " + totalTime + "ms. Average search time = "
				+ (float) totalTime / (float) arr.length + "ms.");
	}
